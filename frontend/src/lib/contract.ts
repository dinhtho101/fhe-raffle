...
export const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTRACT_ADDRESS || '0xc5b526CEDe5A87Da062bbaE73D1DdB2638BfDca5';
...
export class RaffleContract {
  ...
  constructor(signer: ethers.Signer) {
    this.signer = signer;
    // Only create contract instance if properly configured
    if (isContractConfigured()) {
      this.contract = new ethers.Contract(CONTRACT_ADDRESS, RAFFLE_CONTRACT_ABI, signer);
    }
  }
  ...
  private ensureContract(): ethers.Contract {
    ...
    if (!this.contract) {
      ...
      this.contract = new ethers.Contract(normalizedAddress, RAFFLE_CONTRACT_ABI, this.signer);
    }
   ...
   */
  async createRaffle(params: CreateRaffleParams): Promise<ethers.ContractTransaction> {
   ...
   */
  private async createRaffleWithFHEVM(params: CreateRaffleParams): Promise<ethers.ContractTransaction> {
    const contract = this.ensureContract();
    const { name, description, ticketPrice, totalTickets, duration } = params;
    ...
    // Convert base64 string to hex bytes for contract
    const encryptedBytes = ethers.getBytes(`0x${Buffer.from(encryptedTicketPrice, 'base64').toString('hex')}`);
   ...
   */
  private async createRaffleRegular(params: CreateRaffleParams): Promise<ethers.ContractTransaction> {
    const contract = this.ensureContract();
    const { name, description, ticketPrice, totalTickets, duration } = params;
   ...
   */
  async buyTickets(params: BuyTicketsParams): Promise<ethers.ContractTransaction> {
    const contract = this.ensureContract();
   ...
   * End a raffle
   */
  async endRaffle(raffleId: string): Promise<ethers.ContractTransaction> {
    const contract = this.ensureContract();
   ...
   */
  async claimPrize(raffleId: string): Promise<ethers.ContractTransaction> {
    const contract = this.ensureContract();
   ...
   */
  async claimRefund(raffleId: string): Promise<ethers.ContractTransaction> {
    const contract = this.ensureContract();
    return await contract.claimRefund(raffleId);
  }

  /**
   * Get raffle information (FHEVM encrypted)
   */
  async getRaffleInfo(raffleId: string): Promise<RaffleInfo> {
    const contract = this.ensureContract();
    const result = await contract.getRaffleInfoEncrypted(raffleId);
  ...
  /**
   * Get participant's ticket count for a raffle
   */
  async getParticipantTickets(raffleId: string, participant: string): Promise<number> {
    try {
      const contract = this.ensureContract();
      const result = await contract.getParticipantTickets(raffleId, participant);
  ...
  async getRaffleParticipants(raffleId: string): Promise<string[]> {
    try {
      const contract = this.ensureContract();
      return await contract.getRaffleParticipants(raffleId);
    } catch (error) {
