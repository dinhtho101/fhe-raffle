...
export default function RaffleDashboard() {
  ...
  // Auto-refresh every 10 seconds to sync with other users' transactions
  useEffect(() => {
    if (signer) {
      console.log('🔄 Dashboard mounted, loading raffles...');
      loadRaffles(false) // Initial load
      
      // Set up auto-refresh interval
      const intervalId = setInterval(() => {
        loadRaffles(true) // Auto-refresh (don't show loading spinner)
      }, 10000) // Refresh every 10 seconds
      
      return () => {
        console.log('🗑️ Cleaning up dashboard auto-refresh interval');
  ...
  // This ensures data refreshes when user presses F5 or returns to tab
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden && signer) {
        console.log('👁️ Dashboard became visible, refreshing raffles...');
        loadRaffles(false);
      }
    };
    ...
    const handleFocus = () => {
      if (signer) {
        console.log('🔍 Dashboard window focused, refreshing raffles...');
        loadRaffles(false);
      }
    };
  ...
  const loadRaffles = async (isAutoRefresh = false) => {
    if (!signer) return
    ...
    try {
      // Log refresh attempt
      console.log(`🔄 Loading all raffles (auto-refresh: ${isAutoRefresh}) at ${new Date().toLocaleTimeString()}`);
        ...
        {error && (
          <div className="mb-8 bg-red-900/20 border border-red-700 rounded-lg p-6">
            ...
            <div className="mt-4 flex gap-2">
              <Button onClick={() => loadRaffles(false)} size="sm" className="bg-red-600 hover:bg-red-700">
                Retry
              </Button>
              <Button 
                onClick={() => {
